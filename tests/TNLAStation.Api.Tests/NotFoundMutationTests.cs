...
/// になる。これを逆手に取って、EPGStation が明示的に存在チェックをしている操作 (stream の keep、
...
public sealed class NotFoundMutationTests : IDisposable
{
    ...
    {
        // EPGStation (StreamManageModel.keep) は該当ストリームが無いと StreamIsUndefined を投げる。
    ...
    {
        // EPGStation (ThumbnailManageModel.delete) は該当サムネイルが無いと ThumbnailIsNotFound を投げる。
    ...
    {
        // EPGStation (VideoApiModel.getDuration) は 404 ではなく VideoFileIsUndefined 例外にしており、
    ...
    {
        // EPGStation (VideoApiModel.sendToKodi) は送り先名が config に無いと KodiHostIsUndefined を
