...
public sealed class InMemoryEpgRepository : IEpgRepository, IEpgStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        lock (gate)
        {
            List<EpgProgram> found = [];
            foreach (long id in programIds)
            {
                if (programs.TryGetValue(id, out EpgProgram? program))
                {
                    found.Add(program);
                }
            }

            return ValueTask.FromResult<IReadOnlyList<EpgProgram>>(found);
        }
    }
