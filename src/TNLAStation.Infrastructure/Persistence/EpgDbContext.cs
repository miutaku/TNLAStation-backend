...
public sealed class EpgDbContext(DbContextOptions<EpgDbContext> options) : DbContext(options)
{
    ...
    public DbSet<DropLogFileEntity> DropLogFiles => Set<DropLogFileEntity>();

    public DbSet<RecordedHistoryEntity> RecordedHistory => Set<RecordedHistoryEntity>();
    ...
    internal static void ConfigureModel(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ReserveStateEntity>(entity =>
        {
            ...
            entity.Property(item => item.EditJson).HasColumnName("edit_json");
            entity.Property(item => item.CreatedAt).HasColumnName("created_at").HasColumnType("timestamp with time zone");
        });
        ...
        modelBuilder.Entity<RecordedEntity>(entity =>
        {
            ...
            entity.Property(item => item.ProgramId).HasColumnName("program_id");
            entity.Property(item => item.RuleId).HasColumnName("rule_id");
            entity.Property(item => item.ReserveId).HasColumnName("reserve_id");
            entity.Property(item => item.ReserveKey).HasColumnName("reserve_key");
            entity.Property(item => item.ManualReserveId).HasColumnName("manual_reserve_id");
            entity.Property(item => item.ChannelId).HasColumnName("channel_id");
            entity.Property(item => item.StartAt).HasColumnName("start_at").HasColumnType("timestamp with time zone");
            entity.Property(item => item.EndAt).HasColumnName("end_at").HasColumnType("timestamp with time zone");
            ...
            entity.HasIndex(item => item.ReserveKey).HasDatabaseName("ix_recorded_reserve_key");
        ...
        modelBuilder.Entity<DropLogFileEntity>(entity =>
        {
            entity.ToTable("drop_log_files");
            entity.HasKey(item => item.Id).HasName("pk_drop_log_files");
            entity.Property(item => item.Id).HasColumnName("id").ValueGeneratedOnAdd();
            entity.Property(item => item.RecordedId).HasColumnName("recorded_id");
            entity.Property(item => item.ParentDirectoryName).HasColumnName("parent_directory_name");
            entity.Property(item => item.Filename).HasColumnName("filename");
            entity.Property(item => item.ErrorCount).HasColumnName("error_cnt");
            entity.Property(item => item.DropCount).HasColumnName("drop_cnt");
            entity.Property(item => item.ScramblingCount).HasColumnName("scrambling_cnt");
            entity.Property(item => item.CreatedAt).HasColumnName("created_at").HasColumnType("timestamp with time zone");
            entity.HasIndex(item => item.RecordedId).IsUnique().HasDatabaseName("uq_drop_log_files_recorded");
            entity.HasOne(item => item.Recorded)
                .WithOne(recorded => recorded.DropLogFile)
                .HasForeignKey<DropLogFileEntity>(item => item.RecordedId)
                .OnDelete(DeleteBehavior.Cascade)
        ...
        modelBuilder.Entity<RecordedHistoryEntity>(entity =>
        {
            entity.ToTable("recorded_history");
            entity.HasKey(item => item.Id).HasName("pk_recorded_history");
            entity.Property(item => item.Id).HasColumnName("id").ValueGeneratedOnAdd();
            entity.Property(item => item.Name).HasColumnName("name");
            entity.Property(item => item.ChannelId).HasColumnName("channel_id");
            entity.Property(item => item.EndAt).HasColumnName("end_at").HasColumnType("timestamp with time zone");
            entity.HasIndex(item => item.EndAt).HasDatabaseName("ix_recorded_history_end_at");
        });
