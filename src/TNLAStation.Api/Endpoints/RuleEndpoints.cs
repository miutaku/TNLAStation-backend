...
internal static class RuleEndpoints
{
    public static IEndpointRouteBuilder MapRuleEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        rules.MapGet("/{ruleId:long}", GetRuleAsync)
            .WithName("GetRule")
            .WithSummary("ルール取得")
            .WithTags("rules")
        ...
        rules.MapPut("/{ruleId:long}", UpdateRuleAsync)
            .WithName("UpdateRule")
            .WithSummary("ルール更新")
            .WithTags("rules")
            .Accepts<AddRuleRequest>("application/json")
        ...
        rules.MapDelete("/{ruleId:long}", DeleteRuleAsync)
            .WithName("DeleteRule")
            .WithSummary("ルール削除")
            .WithTags("rules")
            .Produces<ResultCodeResponse>();

        rules.MapPut("/{ruleId:long}/enable", EnableRuleAsync)
            .WithName("EnableRule")
            .WithSummary("ルール有効化")
            .WithTags("rules")
            .Produces<ResultCodeResponse>();

        rules.MapPut("/{ruleId:long}/disable", DisableRuleAsync)
    ...
    private static async Task<IResult> AddRuleAsync(
        ...
        CancellationToken cancellationToken)
    {
        long ruleId = await repository.AddAsync(request.ToRule(), cancellationToken);
    ...
    private static async Task<IResult> UpdateRuleAsync(
        ...
        CancellationToken cancellationToken)
    {
        await repository.UpdateAsync(request.ToRule(ruleId), cancellationToken);
        return Ok();
    }
