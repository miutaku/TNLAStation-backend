...
internal static class IptvEndpoints
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (EpgChannel channel in channels)
        {
            ...
            builder.Append(CultureInfo.InvariantCulture, $"tvg-logo=\"{origin}/api/channels/{id}/logo\" ");
            builder.Append(CultureInfo.InvariantCulture, $"group-title=\"{channel.ChannelType}\",{channel.Name}\n");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<EpgProgram> programs = await repository.FindProgramsAsync(
            new EpgScheduleQuery(now, now.AddDays(Math.Clamp(days, 1, 8)), ["GR", "BS", "CS", "SKY"]),
            cancellationToken);
        ...
        await using (XmlWriter writer = XmlWriter.Create(output, settings))
        {
            ...
            await writer.WriteAttributeStringAsync(null, "generator-info-name", null, "TNLAStation");
            ...
            foreach (EpgChannel channel in channels)
            {
                ...
                await WriteTextElementAsync(writer, "display-name", channel.HalfWidthName);
            ...
            foreach (EpgProgram program in programs)
            {
                ...
                await WriteTextElementAsync(writer, "title", program.HalfWidthName);
                if (!string.IsNullOrWhiteSpace(program.HalfWidthDescription))
                {
                    await WriteTextElementAsync(writer, "desc", program.HalfWidthDescription);
                }
